...
def triangle_patch_of_hair(x, y, triangle_side, angle, color_triangle):
...
def face(x, y, radius_face):
    dr.circle(screen, BLACK, (x, y), radius_face + 2)
    dr.circle(screen, SKIN_COLOR, (x, y), radius_face)


def mouth(x, y, radius_mouth):
    dr.polygon(screen, MOUTH_COLOR, [(x, y + radius_mouth * 2 / 3), (x + radius_mouth / 2, y + radius_mouth / 3),
                                     (x - radius_mouth / 2, y + radius_mouth / 3)])
    dr.polygon(screen, BLACK, [(x, y + radius_mouth * 2 / 3), (x + radius_mouth / 2, y + radius_mouth / 3),
...
def nose(x, y, radius_nose):
    dr.polygon(screen, NOSE_COLOR, [(x, y + radius_nose / 8), (x + radius_nose / 40, y),
                                    (x - radius_nose / 40, y)])
    dr.polygon(screen, BLACK, [(x, y + radius_nose / 8), (x + radius_nose / 40, y),
...
def hair(x, y, radius_hair, color_hair=(175, 19, 207)):
    ...
    for i in range(number_hair + 1):
        for j in range(i // 2, 0, -1):
            triangle_patch_of_hair(x - radius_hair * np.sin(angle_hair * (j - 0.5) / 180 * np.pi),
...
def python_is_amazing():
    dr.rect(screen, (151, 245, 100), (0, 0, int(400 * SIZE * 1.75), int(60 * SIZE)))
    dr.rect(screen, BLACK, (0, 0, int(400 * SIZE * 1.75), int(60 * SIZE)), int(2 * SIZE))
    font = pygame.font.Font(None, 72)
    text = font.render(
        "ANTHON is  REALLY AMAZING!", True, BLACK)
    place = text.get_rect(
...
FPS = 30
screen = pygame.display.set_mode((int(400 * SIZE * 1.75), int(400 * SIZE)))
